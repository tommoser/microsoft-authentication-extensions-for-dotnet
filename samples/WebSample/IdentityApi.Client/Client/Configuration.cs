...
namespace IdentityApi.Client.Client
{
        ...
        public class Configuration : IReadableConfiguration
    {
        ...
        public static readonly ExceptionFactory DefaultExceptionFactory = (methodName, response) =>
        {
            ...
            if (status >= 400)
            {
                return new ApiException(status,
                    string.Format(CultureInfo.InvariantCulture, "Error calling {0}: {1}", methodName, response.Content),
                    response.Content);
            }
            if (status == 0)
            {
                return new ApiException(status,
                    string.Format(CultureInfo.InvariantCulture, "Error calling {0}: {1}", methodName, response.ErrorMessage), response.ErrorMessage);
            }
            return null;
        };
        ...
        {
            string report = "C# SDK (IdentityApi.Client) Debug Report:\n";
